...
export function Headline({ main, prop, fcTop }: { main: Station[]; prop: any; fcTop?: { name: string; mm: number } | null }) {
  ...
  if (next) parts.push(<span key="p">Estimación estadística: la subida podría notarse en <b>{next.to_name}</b> en ~{Math.round(Math.max(next.hours_from_now[0], 0))}–{Math.round(next.hours_from_now[1])} h. </span>);
